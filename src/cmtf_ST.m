...
%   Y = [[gamma; A, D]]
%
% We assume that the first mode factor A is shared.
%
% Input:
...
function [tensor_cp, matrix_pred, rec_errors] = ...
    ...
    %    based on it to initialize the first mode A.
    %---------------------------------------------
    unfoldX_mode1 = double(tenmat(tensor_3d, 1));
    coupled_unfold = [unfoldX_mode1, matrix_2d];
    
    coupled_init = initialize_cp(coupled_unfold, rankCP, initType);
    
    % Replace only the first mode factor
    tensor_cp.factors{1} = coupled_init.factors{1};
    ...
    for iter = 1:nIterMax
        ...
        A_factor = tensor_cp.factors{1};
        D_temp = A_factor \ matrix_2d;
        D = D_temp';
        ...
        for jj = nDim:-1:1
            ...
            % If it is the first mode (jj == 1), then concatenate with the matrix
            if jj == 1
                kr = [kr; D];
        ...
        Y_pred = tensor_cp.factors{1} * D';  % A * D' (size: I1 x J)
